(function() {
  var CodeCatalog, MEMBER_EXPRESSION, Model, NODE_TYPES, OBJECT_EXPRESSION, RETURN_STATEMENT, RSVP, acorn, astUtils, findSingletonDefinitions, findSingletonInitMethod, fs, nullFn, q, walk, walkTopLevelSingleton, _;
  ...
  findSingletonInitMethod = function(if_node) {
    var init, instance, test;
    test = if_node.test;
  ...
  walkTopLevelSingleton = function(node, singletons) {
  ...
  findSingletonDefinitions = function(ast) {
    var singletons;
    singletons = new CodeCatalog();
    astUtils.nodeWalk(ast, nullFn, {
      VariableDeclaration: function(node) {
        return walkTopLevelSingleton(node, singletons);
      },
